...
bool existencia_racao(char *filename)
{
    ...
    int tentativas = 5;
    Mat centros;
    kmeans(amostras, clusterCount, labels, TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 10000, 0.0001), tentativas, KMEANS_PP_CENTERS, centros);
    ...
    if ((float)((1.0 * pixeis_racao) / (1.0 * pixeis_tigela) > 0.2))
